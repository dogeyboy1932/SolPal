...
interface NodeContextValue {
  ...
  addToAccessibleNodes: (nodeId: string, accessible: boolean) => void;
...
export const NodeProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [llmAccessibleNodeIds, setLlmAccessibleNodeIds] = useState<Set<string>>(
    new Set()
  );
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      console.log(
        '🔗 NodeManagement MCP functions initialized with',
        nodes.length,
        'nodes (',
        llmAccessibleNodeIds.size,
        'LLM accessible)'
      );
    }
  }, [nodes, llmAccessibleNodeIds, isLoading]);
      ...
      if (storedData) {
        ...
        const hydratedNodes = parsedNodes.map((node: any) => ({
          ...
          ...(node.type === 'event' && {
            ...
            lastTransactionDate: node.lastTransactionDate
              ? new Date(node.lastTransactionDate)
              : undefined,
          }),
          ...(node.type === 'person' && {
            lastTransactionDate: node.lastTransactionDate
  ...
  const saveLLMAccessSettings = async (accessSet: Set<string>) => {
    try {
      await AsyncStorage.setItem(
        LLM_ACCESS_STORAGE_KEY,
        JSON.stringify(Array.from(accessSet))
      );
    } catch (err) {
  ...
  const addToAccessibleNodes = useCallback(
  ...
  const toggleAllNodesLLMAccess = useCallback(
    (accessible: boolean) => {
      if (accessible) {
  ...
  const createPersonNode = useCallback(
    async (data: CreatePersonNodeData): Promise<PersonNode> => {
  ...
  const createEventNode = useCallback(
    async (data: CreateEventNodeData): Promise<EventNode> => {
  ...
  const createCommunityNode = useCallback(
    async (data: CreateCommunityNodeData): Promise<CommunityNode> => {
  ...
  const updatePersonNode = useCallback(
    async (id: string, data: UpdatePersonNodeData) => {
      setNodes(prev =>
        prev.map(node =>
  ...
  const updateEventNode = useCallback(
    async (id: string, data: UpdateEventNodeData) => {
      setNodes(prev =>
        prev.map(node =>
  ...
  const updateCommunityNode = useCallback(
    async (id: string, data: UpdateCommunityNodeData) => {
      setNodes(prev =>
        prev.map(node =>
  ...
  const setNodeActive = useCallback(
    async (id: string, isActive: boolean) => {
      setNodes(prev =>
        prev.map(node =>
          node.id === id ? { ...node, isActive, updatedAt: new Date() } : node
        )
      );
